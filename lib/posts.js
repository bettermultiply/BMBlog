...
export function getSortedPostsData() {
  ...
  const fileNames = dirNames.map((pid) => {
    const directory = path.join(postsDirectory, pid);
  ...
  const all = fileNames.map(({filesName, directory, pid}) => {
...
export function getAllPostDir() {
  const pid = fs.readdirSync(postsDirectory);
  return (
    pid.map((pid) => {
      return {
        params: {
...
export function getSpecialIds(pid) {
  const partPath = path.join(postsDirectory, pid);
  const id = fs.readdirSync(partPath);
  const ids = id.map((id) => {
...
export function getDirPostData(pid) {    
  // Get file names under /posts
  const directory = path.join(postsDirectory, pid);
